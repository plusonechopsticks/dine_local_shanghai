...
// Dragon's listing ID is 390001
// Guest: Patrice Tobler, Patricetobler@icloud.com
// Date: August 23, 2026 at 19:00 CST
// GYG ref: GYGZGZXYWLXY
// 4 adults

const diningDate = new Date('2026-08-23T19:00:00+08:00').getTime();
const now = Date.now();

// Insert booking
const [result] = await db.execute(`
  INSERT INTO bookings (
    hostListingId, guestName, guestEmail, guestCount,
    diningDate, mealType, status, paymentStatus,
    bookingSource, specialRequests, createdAt, updatedAt
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`, [
  390001,
  'Patrice Tobler',
  'Patricetobler@icloud.com',
  4,
  diningDate,
  'dinner',
  'confirmed',
  'paid',
  'getyourguide',
  'GYG booking reference: GYGZGZXYWLXY. Note from booker Petra: booked as a wedding gift for son Patrice and his friends.',
  now,
  now
]);
...
// Get Dragon's host info
const [rows] = await db.execute('SELECT hostName, email FROM host_listings WHERE id = 390001');
const host = rows[0];
console.log('Host:', host.hostName, host.email);
          ...
          <div style="background-color:#fef3c7;border-left:4px solid #7c2d12;padding:20px;border-radius:4px;">
            ...
            <table width="100%" cellpadding="8" cellspacing="0">
              <tr>
                ...
                <td style="color:#1f2937;font-size:14px;">Dragon (Xiaolong &#x5C0F;&#x9F99;)</td>
              </tr>
              <tr>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
        ...
        <tr><td style="padding:0 40px 30px 40px;">
          <div style="background-color:#fef2f2;border-left:4px solid #dc2626;padding:20px;border-radius:4px;">
            <h3 style="margin:0 0 12px 0;color:#dc2626;font-size:16px;font-weight:600;">&#x26A0;&#xFE0F; Special Note from Booker</h3>
...
console.log('Guest email sent:', JSON.stringify(guestResult));
...
console.log('Host email sent to', host.email, ':', JSON.stringify(hostResult));
