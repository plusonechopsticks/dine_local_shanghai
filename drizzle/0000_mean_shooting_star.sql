...
CREATE TABLE `testimonials` (
	...
	`travelerType` varchar(255) NOT NULL,
	`previewText` text NOT NULL,
	`fullText` text NOT NULL,
	`photoUrls` json NOT NULL,
	`photoAlts` json NOT NULL,
	`hostDate` varchar(100) NOT NULL,
...
ALTER TABLE `page_views` ADD CONSTRAINT `page_views_hostListingId_host_listings_id_fk` FOREIGN KEY (`hostListingId`) REFERENCES `host_listings`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `testimonials` ADD CONSTRAINT `testimonials_hostListingId_host_listings_id_fk` FOREIGN KEY (`hostListingId`) REFERENCES `host_listings`(`id`) ON DELETE no action ON UPDATE no action;