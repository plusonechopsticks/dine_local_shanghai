...
export default function BookingSuccess() {
  ...
  // Look up the booking by Stripe session ID so we have bookingId + guest info
  const { data: bookingData } = trpc.payment.getBookingBySessionId.useQuery(
    { sessionId: sessionId! },
    { enabled: !!sessionId }
  );
  ...
  // If survey already answered, skip straight to success content
  useEffect(() => {
    if (hasResponded?.responded) setSurveyDone(true);
  }, [hasResponded]);

  // Show survey first (if we have a bookingId and survey not yet done)
  const showSurvey = !!bookingId && !surveyDone;
              ...
              <CardContent className="pt-6 text-center space-y-6">
                ...
                <div className="space-y-3 pt-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <Button onClick={() => setLocation("/hosts")} className="w-full">
                      Browse More Hosts
                    </Button>
